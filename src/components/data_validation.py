...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logger.info(f'Dataframe number of columns: {len(dataframe.columns)}')
    ...
    def detect_data_drift(self, base_df: pd.DataFrame, current_df: pd.DataFrame, threshold=0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                ...
                goodness_of_fit = ks_2samp(d1, d2)
                if threshold <= goodness_of_fit.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status = False
                report.update({
                    column: {
                        "p_value": float(goodness_of_fit.pvalue),
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = 'Train dataframe does not contain all the required columns! \n'
            status = self.validate_number_of_columns(test_df)
            if not status:
                error_message = 'Test dataframe does not contain all the required columns! \n'
